...
export default async function GetRecipes(
  ...
  FROM 
      dishes 
  LEFT JOIN 
      dish_categories ON dishes.category_id = dish_categories.id 
  LEFT JOIN 
      users ON dishes.user_id = users.id 
  LEFT JOIN 
      ingredients ON ingredients.dish_id = dishes.id 
  LEFT JOIN 
      recipes ON recipes.dish_id = dishes.id
  GROUP BY 
      ...
      HAVING 
    COUNT(ingredients.id) > 0 AND COUNT(recipes.id) > 0;`;
